...
class WhitespaceTokenizer(object):
    ...
    def __call__(self, text):
        words = text.split()
        # All tokens 'own' a subsequent space character in this tokenizer所有标记都“拥有”此标记符中的后续空格字符
...
def dependency_adj_matrix(text):
    ...
    for token in tokens:
        matrix[token.i][token.i] = 1  # 自己到自己的闭环标记为1
        for child in token.children:
            matrix[token.i][child.i] = 1  # 自己到孩子的闭环标记为1
...
def process(filename):
    ...
    pickle.dump(idx2graph, fout)  # 序列化（封装）对象，将对象idx2graph保存到文件fout中去。
    fout.close() 
