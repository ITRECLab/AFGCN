...
def load_word_vec(path, word2idx=None, embed_dim=300):
    ...
    word_vec = {}
    for line in fin:
        tokens = line.rstrip().split()  # rstrip()  方法用于删除字符串尾部指定的字符
...
def build_embedding_matrix(word2idx, embed_dim, type):
    ...
    if os.path.exists(embedding_matrix_file_name):
        ...
        # 第一行的所有元素  均匀分布(low,high,size)  size 为输出样本数目，输出1*embed_dim个样本
        ...
        pickle.dump(embedding_matrix, open(embedding_matrix_file_name, 'wb'))  # # 序列化对象，将对象obj（前）保存到文件file中去。
...
class ABSADatesetReader:
    ...
    def __read_text__(fnames):  # 从数据集中读取数据
        text = ''
        for fname in fnames:
    ...
    def __read_data__(fname, tokenizer):
        ...
        fin = open(fname + '.graph_con_a', 'rb')
        idx2graph = pickle.load(fin)
        fin.close()
        fin = open(fname+'.tree', 'rb')
        #fin = open(fname+'.tree_con_a', 'rb')
        ...
        for i in range(0, len(lines), 3):
            ...
            text_indices = tokenizer.text_to_sequence(text_left + " " + aspect + " " + text_right)  #把文本转化为数字序列
            ...
            dependency_dist = idx2dist[i].tolist()
    ...
    def __init__(self, dataset='twitter', embed_dim=300):
        ...
        if os.path.exists(dataset+'_word2idx.pkl'):
            ...
            tokenizer.fit_on_text(text)  # 使用一系列文档来生成token词典，texts为list类
            with open(dataset+'_word2idx.pkl', 'wb') as f:
                 pickle.dump(tokenizer.word2idx, f)  # 序列化对象，将对象obj（前）保存到文件f中去。
        self.embedding_matrix = build_embedding_matrix(tokenizer.word2idx, embed_dim, dataset)  # 建立嵌入矩阵
